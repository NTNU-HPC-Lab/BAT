...
class GccFlagsTuner(MeasurementInterface):
  ...
  def run(self, desired_result, input, limit):
    ...
    make_bfs = 'nvcc -gencode=arch=compute_' + cc + ',code=sm_' + cc + ' -I ' + start_path + '/cuda-common -I ' + start_path + '/common -g -O2 -c ' + start_path + '/bfs/BFS.cu'
    make_bfs += ' -D{0}={1} \n'.format('BLOCK_SIZE',cfg['BLOCK_SIZE'])
    make_bfs += 'nvcc -gencode=arch=compute_' + cc + ',code=sm_' + cc + ' -I ' + start_path + '/cuda-common -I ' + start_path + '/common -g -O2 -c ' + start_path + '/bfs/bfs_kernel.cu'
    make_bfs += ' -D{0}={1}'.format('UNROLL_OUTER_LOOP',cfg['UNROLL_OUTER_LOOP'])
    make_bfs += ' -D{0}={1}'.format('UNROLL_INNER_LOOP',cfg['UNROLL_INNER_LOOP'])
    make_bfs += ' -D{0}={1} \n'.format('CHUNK_SIZE',cfg['CHUNK_SIZE'])

    if args.parallel: 
      make_paralell_start = 'mpicxx -I ' + start_path + '/common/ -I ' + start_path + '/cuda-common/ -I /usr/local/cuda/include -DPARALLEL  -I ' + start_path + '/mpi-common/ -g -O2 -c -o Graph.o ' + start_path + '/common/Graph.cpp \n'
      ...
      make_paralell_end = 'mpicxx -L ' + start_path + '/cuda-common -L ' + start_path + '/common -o BFS Graph.o main.o BFS.o bfs_kernel.o -lSHOCCommon "-L/usr/local/cuda/bin/../targets/x86_64-linux/lib/stubs" "-L/usr/local/cuda/bin/../targets/x86_64-linux/lib" -lcudadevrt -lcudart_static -lrt -lpthread -ldl -lrt -lrt'
      compile_cmd = make_paralell_start + make_bfs + make_paralell_end
    else: 
      make_serial_start = 'nvcc -I ' + start_path + '/common/ -I ' + start_path + '/cuda-common/ -g -O2 -c -o Graph.o ' + start_path + '/common/Graph.cpp \n'
      make_serial_start += 'nvcc -I ' + start_path + '/common/ -I ' + start_path + '/cuda-common/ -g -O2 -c -o main.o ' + start_path + '/cuda-common/main.cpp \n'
      make_serial_end = 'nvcc -L ' + start_path + '/cuda-common -L ' + start_path + '/common -o BFS Graph.o main.o BFS.o bfs_kernel.o -lSHOCCommon'
      compile_cmd = make_serial_start + make_bfs + make_serial_end
    
    compile_result = self.call_program(compile_cmd)
    assert compile_result['returncode'] == 0

    bfs_command = './BFS -s ' + str(args.problem_size)
    if args.parallel: 
