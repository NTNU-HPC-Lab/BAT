import os
import json
from bat.manager.util import benchmark_root_path

for folder_name in os.listdir(f"{benchmark_root_path}/"):
    try:
        with open(f"{benchmark_root_path}/{folder_name}/{folder_name}-CAFF.json", 'r') as f:
            text = f.read()
            j = json.loads(text)
