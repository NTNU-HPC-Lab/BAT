...
scan_single_block_helper(
    float* __restrict__ g_block_sums_single, 
    double* __restrict__ g_block_sums_double,
    const int n
){
    #if PERCISION == 32
        scan_single_block(g_block_sums_single, n)
    #else 
        scan_single_block(g_block_sums_double, n)
    #endif
}
