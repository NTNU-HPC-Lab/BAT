...
from src.reader.util import get_data_path
...
def populate_args(kernel_spec):
    return [populate_data(arg, kernel_spec) for arg in kernel_spec["arguments"]]
...
def handle_vector_data(arg, kernel_spec):
    # random.seed(10)
    t = arg["fillType"]
    if t == "file":
        with open(get_data_path(kernel_spec, arg["path"]), 'r') as f:
...
def populate_data(arg, kernel_spec):
    m = arg["memoryType"].lower()
    if m == "vector":
        return handle_vector_data(arg, kernel_spec)
    if m == "scalar":
...
def get_launch_config(kernel_spec, tuning_config):
    ...
    if kernel_spec.get("iterations"):
        launch_config["ITERATIONS"] = eval(str(kernel_spec["iterations"]))
